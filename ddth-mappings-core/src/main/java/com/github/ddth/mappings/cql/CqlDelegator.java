...
public class CqlDelegator {
    ...
    private final static String[] _COLS_ALL = { COL_MAPPING, COL_NAMESPACE, COL_KEY, COL_VALUE };
    private final static String[] _WHERE_STATS = { COL_MAPPING + "=?", COL_NAMESPACE + "=?",
            COL_KEY + "=?" };
    private final static String CQL_UPDATE_STATS = "UPDATE {0} SET " + COL_VALUE + "=" + COL_VALUE
            + "+? WHERE " + StringUtils.join(_WHERE_STATS, " AND ");
    private final static String CQL_SELECT_ALL_STATS = "SELECT " + StringUtils.join(_COLS_ALL, ",")
    ...
    public ResultSet updateStats(String mapping, String namespace, String key, long value) {
        ...
        if (pstmUpdateStats == null) {
            pstmUpdateStats = prepareStatement(session,
                    MessageFormat.format(CQL_UPDATE_STATS, tableStats));
        }
    ...
    /**
     * Fetch a column value from C* row.
     * 
     * @param row
     * @param colName
     * @param clazz
    ...
    public <X> X fetchValue(Row row, String colName, Class<X> clazz) {
        Object result = null;
        if (clazz == boolean.class || clazz == Boolean.class) {
            result = row.getBool(colName);
        } else if (clazz == byte.class || clazz == Byte.class) {
            result = row.getByte(colName);
        } else if (clazz == short.class || clazz == Short.class) {
            result = row.getShort(colName);
        } else if (clazz == int.class || clazz == Integer.class) {
            result = row.getInt(colName);
        } else if (clazz == long.class || clazz == Long.class) {
            result = row.getLong(colName);
        } else if (clazz == float.class || clazz == Float.class) {
            result = row.getFloat(colName);
        } else if (clazz == double.class || clazz == Double.class) {
            result = row.getDouble(colName);
        } else if (clazz == String.class) {
            result = row.getString(colName);
        } else if (clazz == byte[].class) {
            result = row.getBytes(colName).array();
        }
        return (X) result;
    }

    /**
     * Fetch and decode array of values from C* row.
     * 
     * @param row
     * @param colName
     * @param clazz
     * @return
     */
    public Object[] fetchAndDecodeValues(Row row, String colName, Class<?>... clazz) {
        String[] tokens = MappingsUtils.seSplit(row.getBytes(colName));
        Object[] result = new Object[tokens.length];
        for (int i = 0; i < tokens.length; i++) {
            result[i] = SerializationUtils.fromJsonString(tokens[i], clazz[i]);
        }
        return result;
    }
